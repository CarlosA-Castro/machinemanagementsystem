...
def get_machine_token(maquina_id):
    """
    Retorna el token de autenticación ESP32 de una máquina.
    Solo accesible por admins — el token se usa para configurar el firmware.
    """
    connection = None
    cursor = None
    try:
        ...
        cursor.execute(
            "SELECT id, name, machine_token FROM machine WHERE id = %s",
            (maquina_id,)
        )
...
def regenerar_machine_token(maquina_id):
    """
    Genera un nuevo token para la máquina e invalida el anterior.
    Usar cuando el token se haya comprometido o al reemplazar un ESP32.
    """
    import uuid
    connection = None
    cursor = None
    try:
        ...
        nuevo_token = uuid.uuid4().hex  # 32 chars hex sin guiones
        cursor.execute(
            "UPDATE machine SET machine_token = %s WHERE id = %s",
            (nuevo_token, maquina_id)
        )
        connection.commit()

        logger.info(
            'Token ESP32 regenerado — máquina: %s (id=%s) por usuario: %s',
            machine['name'], maquina_id, session.get('user_name', '?')
        )
        ...
        return jsonify({
            ...
            'machine_name': machine['name'],
            'token':        nuevo_token,
            'message':      'Token regenerado. Actualiza el firmware del ESP32 con el nuevo token.',
        })
    finally:
        if cursor:     cursor.close()
        if connection: connection.close()
